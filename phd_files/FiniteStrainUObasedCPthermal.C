...
#include "FiniteStrainUObasedCPthermal.h"
...
registerMooseObject("TensorMechanicsApp", FiniteStrainUObasedCPthermal);

defineLegacyParams(FiniteStrainUObasedCPthermal);

InputParameters
FiniteStrainUObasedCPthermal::validParams()
{
  InputParameters params = ComputeStressBase::validParams();
  params.addClassDescription("UserObject based Crystal Plasticity system with thermal strains.");
  ...
  params.addCoupledVar("phi","Coupled phase field variable");
  params.addRequiredCoupledVar("stress_free_temperature","Stress free temperature");
  params.addRequiredParam<std::vector<Real>>("alpha", "Vector of values defining the thermal expansion");
  params.addRequiredCoupledVar("beta","Volumetric strain factor");
  return params;
}

FiniteStrainUObasedCPthermal::FiniteStrainUObasedCPthermal(const InputParameters & parameters)
  : ComputeStressBase(parameters),
   ...
   _fth(declareProperty<RankTwoTensor>("fth")), //thermal deformation gradient
   _fth_old(getMaterialPropertyOld<RankTwoTensor>(
        "fth")), // thermal deformation gradient of previous increment
   _ftr(declareProperty<RankTwoTensor>("ftr")),
   _ftr_old(getMaterialPropertyOld<RankTwoTensor>("ftr")),
    _T(coupledValue("temp")),
    _phi(coupledValue("phi")),
   _T0(coupledValue("stress_free_temperature")),
  ...
  
{
  ...
  _delta_dfgrd.zero();
  _alpha_tensor.fillFromInputVector(getParam<std::vector<Real>>("alpha"));
...
void
FiniteStrainUObasedCPthermal::initQpStatefulProperties()
{
  for (unsigned int i = 0; i < _num_uo_slip_rates; ++i)
  {
  ...
  _fth[_qp].setToIdentity();
  _ftr[_qp].setToIdentity();
 ...
 */
void
FiniteStrainUObasedCPthermal::computeQpStress()
{
  ...
  for (unsigned int i = 0; i < _num_uo_slip_rates; ++i)
    _uo_slip_rates[i]->calcFlowDirection(_qp, (*_flow_direction[i])[_qp]);  //array stores results of the calcflowdirection
...
void
FiniteStrainUObasedCPthermal::preSolveQp()
{
  for (unsigned int i = 0; i < _num_uo_state_vars; ++i)
  ...
  _fth_old_inv = _fth_old[_qp].inverse();
  _ftr_old_inv = _ftr_old[_qp].inverse();
}

void
FiniteStrainUObasedCPthermal::solveQp()
{
  preSolveStatevar();
  solveStatevar();
  if (_err_tol)
...
void
FiniteStrainUObasedCPthermal::postSolveQp()
{
...
void
FiniteStrainUObasedCPthermal::preSolveStatevar()
{
  for (unsigned int i = 0; i < _num_uo_state_vars; ++i)
  ...
  _fp_inv = _fp_old_inv;
  _fth_inv = _fth_old_inv;
  _ftr_inv = _ftr_old_inv;
}

void
FiniteStrainUObasedCPthermal::solveStatevar()
{
  unsigned int iterg;
  bool iter_flag = true;
...
bool
FiniteStrainUObasedCPthermal::isStateVariablesConverged()
{
  Real diff;
...
void
FiniteStrainUObasedCPthermal::postSolveStatevar()
{
  for (unsigned int i = 0; i < _num_uo_state_vars; ++i)
    _state_vars_old_stored[i] = (*_mat_prop_state_vars[i])[_qp];

  _fp_old_inv = _fp_inv;
  _fth_old_inv = _fth_inv;
  _ftr_old_inv = _ftr_inv;
  
}

void
FiniteStrainUObasedCPthermal::preSolveStress()
{
}

void
FiniteStrainUObasedCPthermal::solveStress()
{
  unsigned int iter = 0;
  RankTwoTensor dpk2;
...
void
FiniteStrainUObasedCPthermal::postSolveStress()
{
  _fp[_qp] = _fp_inv.inverse();
  _fth[_qp] = _fth_inv.inverse(); //Added
  _ftr[_qp] = _ftr_inv.inverse();
}

void
FiniteStrainUObasedCPthermal::updateSlipSystemResistanceAndStateVariable()
{
  for (unsigned int i = 0; i < _num_uo_state_vars; ++i)
...
void
FiniteStrainUObasedCPthermal::calcResidJacob()
{
  calcResidual();
  if (_err_tol)
    return;
...
void
FiniteStrainUObasedCPthermal::getSlipRates()
{
  for (unsigned int i = 0; i < _num_uo_slip_rates; ++i)
  {
...
void
FiniteStrainUObasedCPthermal::calcResidual()
{
  RankTwoTensor iden(RankTwoTensor::initIdentity), ce, ee, ce_pk2, eqv_slip_incr, pk2_new, thermal_denom, thermal_denom1, eqv_thermal_slip_incr, eqv_phase_strain;
  Real _alpha_trace;
  getSlipRates();
  if (_err_tol)
    return;
  ...
  for (unsigned int i = 0; i < _num_uo_slip_rates; ++i)
   ...
   _alpha_trace = _alpha_tensor.tr();
  
   
   thermal_denom = (iden + (_alpha_tensor * (_T[_qp] -_T0[_qp])));
   //Moose::out<<"thermal_denom before inv"<<thermal_denom<<"\n";
   thermal_denom1 = thermal_denom.inverse();
   //Moose::out<<"thermal_denom1"<<thermal_denom1<<"\n";
   //eqv_thermal_slip_incr = (_T[_qp]*_alpha_tensor.tr())/(1 + (_alpha_tensor.tr()*(_T[_qp] -_T0[_qp])))*_dt;

   eqv_thermal_slip_incr = (_T[_qp]*_alpha_tensor)*thermal_denom1;
   eqv_phase_strain = (_beta*(_phi[_qp]));
   eqv_slip_incr = iden - eqv_slip_incr;
   eqv_thermal_slip_incr = iden - eqv_thermal_slip_incr;   
   eqv_phase_strain = iden - eqv_phase_strain;

  _fth_inv = _fth_old_inv * eqv_thermal_slip_incr;
   _fp_inv = _fp_old_inv * eqv_slip_incr;
  _ftr_inv = _ftr_old_inv * eqv_phase_strain;
  _fe = _dfgrd_tmp * _fp_inv * _fth_inv * _ftr_inv;
  ...
  //Moose::out<<"fp"<<_fp_inv<<"\n";
  //Moose::out<<"fp type"<<typeid(_fp_inv).name()<<"\n";
}

void
FiniteStrainUObasedCPthermal::calcJacobian()
{
  ...
  for (unsigned int i = 0; i < LIBMESH_DIM; ++i)
	...
	//Moose::out<<"dfedfpinv"<<dfedfpinv<<"\n";
       //Moose::out<<"dfedfpinv type"<<typeid(dfedfpinv).name()<<"\n";
...
void
FiniteStrainUObasedCPthermal::calcTangentModuli()
{
...
void
FiniteStrainUObasedCPthermal::elastoPlasticTangentModuli()
{
...
void
FiniteStrainUObasedCPthermal::elasticTangentModuli()
{
...
bool
FiniteStrainUObasedCPthermal::lineSearchUpdate(const Real rnorm_prev, const RankTwoTensor dpk2)
{
